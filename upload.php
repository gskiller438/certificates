...
if (isset($_GET['action']) && $_GET['action'] === 'delete' && isset($_GET['id'])) {
    $id = intval($_GET['id']);
    $stmt = $conn->prepare("SELECT filename FROM uploads WHERE id=? AND user_id=? LIMIT 1");
...
$stmt = $conn->prepare("SELECT id, filename, uploaded_at FROM uploads WHERE user_id=? ORDER BY uploaded_at DESC");
...
:root {
    --primary: #4174d9ff;
    --primary-hover: #08c908ff;
    --danger: #ff3b3bff;
    --danger-hover: #790081ff;
    --bg: #f3f4f6;
    --card: #ffffff;
    --text: #111827;
...
<body>
<div class="container">
 <center> <h2><..Sports Certificate Gallery..></h2> </center>
  ...
  <h3>Sports Certificate Upload Photos.</h3><br>
  <div class="preview-grid">
    <?php while ($row = $result->fetch_assoc()): ?>
      <div class="preview-card">
        <img src="uploads/<?php echo htmlspecialchars($row['filename']); ?>" alt="photo">
        <div class="file-info">
          <strong><?php echo htmlspecialchars($row['filename']); ?></strong>
          <span><?php echo $row['uploaded_at']; ?></span>
        </div>
        <div class="actions">
          <a class="btn" href="upload.php?action=download&id=<?php echo $row['id']; ?>">Download</a>
          <a class="btn ghost" href="upload.php?action=delete&id=<?php echo $row['id']; ?>" onclick="return confirm('Are you sure?')">Delete</a>
        </div>
      </div>
    <?php endwhile; ?>
  </div>
